...
def find_matching_reads_single_fastq(reads_folder: str) -> Dict[str, str]:
    """
    Find single reads in the specified folder.
    ...
    return _find_matching_reads_single_fastq(reads_folder)
...
def _find_matching_reads_single_fastq(reads_folder: str) -> Dict[str, str]:
    """
    Internal function to find single reads in the specified folder.
    ...
    forward_reads: Dict[str, str] = {}
    reverse_reads: Dict[str, str] = {}
    print("from _find_matching_reads_single_fastq")
    for file_name in os.listdir(reads_folder):
...
def find_matching_reads_paired_fastq(reads_folder: str) -> Dict[str, Tuple[str, str]]:
    """
    Public interface to find matching paired reads in a specified folder.
    ...
    if not ( os.path.exists(reads_folder) and os.path.isdir(reads_folder)):
        raise ValueError(f"The folder path: {reads_folder} doesn't exist or is not a directory. Please select a valid directory")
    return _find_matching_reads_paired_fastq(reads_folder)
...
def _find_matching_reads_paired_fastq(reads_folder: str) -> Dict[str, Tuple[str, str]]:
    """
    Finds matching paired reads in the specified folder.
...
def find_num_single_files_fastq(folder_path):
    return len(find_matching_reads_single_fastq(folder_path))


def find_num_pair_files(folder_path):
    return len(find_matching_reads_paired(folder_path))

def find_num_pair_files_fastq(folder_path):
    return len(find_matching_reads_paired_fastq(folder_path))

def find_num_files(folder_path, is_mode_ab1):
    print("is_mode_ab1", is_mode_ab1)
    if is_mode_ab1:
        return find_num_single_files(folder_path), find_num_pair_files(folder_path)
    else:
        return find_num_single_files_fastq(folder_path), find_num_pair_files_fastq(folder_path)
...
def find_gen_files_fastq(folder_path: str, is_blastnr: bool, is_blastnt: bool, is_blastBoth, matching_func: Callable[[str], Dict[str, Tuple[str, str]]]) -> List[str]:
    """
    Finds and lists directories within the specified folder that meet certain criteria.
    ...
    # if not (blast_option =="Blastnt" or blast_option == "Blastnr" or blast_option == "Both"):
    #     raise ValueError(f"blast_option can have only theses values: Blastnt, Blastnr, Both; however, the provided is {blast_option}")
    return _find_gen_files_fastq(folder_path, is_blastnr, is_blastnt, is_blastBoth, matching_func)
...
def _find_gen_files_fastq(folder_path: str, is_blastnr: bool, is_blastnt: bool, is_blastBoth, matching_func: Callable[[str], Dict[str, Tuple[str, str]]]) -> List[str]:
    """
    Internal function to find directories within the specified folder based on certain criteria.
...
def find_gen_single_files(folder_path: str, is_blastnr: bool, is_blastnt: bool, is_blast_both: bool, is_mode_ab1) -> List[str]:
    """
    ...
    if is_mode_ab1:
        ...
        return _find_gen_single_files_fastq(folder_path, is_blastnr, is_blastnt, is_blast_both)
...
def _find_gen_single_files_fastq(folder_path, is_blastnr: bool, is_blastnt: bool, is_blast_both):
    """
    Internal function to find generated single files based on the specified BLAST option.
    ...
    Returns:
        Set[str]: A set of file names that match the given BLAST option.
    """
    return find_gen_files_fastq(folder_path, is_blastnr, is_blastnt, is_blast_both, find_matching_reads_single_fastq)
...
def find_num_gen_single_files_fastq(folder_path, is_blastnr: bool, is_blastnt: bool, is_blastnt_nr):
    return len(find_gen_files_fastq(folder_path, is_blastnr, is_blastnt, is_blastnt_nr, find_matching_reads_single_fastq))
...
def find_num_gen_pair_files_fastq(folder_path, is_blastnr: bool, is_blastnt: bool, is_blastnt_nr: bool):
    return len(find_gen_files_fastq(folder_path, is_blastnr, is_blastnt, is_blastnt_nr, find_matching_reads_paired_fastq))


def find_num_files_gen(folder_path, is_blastnr: bool, is_blastnt: bool, is_blastnt_nr: bool, is_mode_ab1):
    if is_mode_ab1:
        ...
        return find_num_gen_single_files_fastq(folder_path, is_blastnr, is_blastnt, is_blastnt_nr), find_num_gen_pair_files_fastq(folder_path, is_blastnr, is_blastnt, is_blastnt_nr)
